...
namespace Robust.Client.Physics
{
    public sealed partial class GridFixtureSystem : SharedGridFixtureSystem
    {
        ...
        private sealed class GridSplitNodeOverlay : Overlay
        {
            ...
            public GridSplitNodeOverlay(GridFixtureSystem system, TransformSystem transform, SharedMapSystem map)
            {
                _system = system;
            ...
            protected internal override void Draw(in OverlayDrawArgs args)
            {
                ...
                var state = (_system, _transform, args.WorldBounds, worldHandle, args.MapUid);

                _map.FindGridsIntersecting(args.MapId, args.WorldBounds, ref state,
                    (EntityUid uid, MapGridComponent grid,
                        ref (GridFixtureSystem system, TransformSystem transform, Box2Rotated worldBounds, DrawingHandleWorld worldHandle, EntityUid layerMap) tuple) =>
                    {
                        // May not have received nodes yet.
                        if (!tuple.system._nodes.TryGetValue(uid, out var nodes))
                            return true;

                        if (!tuple.transform.TryGetRenderLayerSample(uid, tuple.layerMap, out var renderLayer))
                            return true;
