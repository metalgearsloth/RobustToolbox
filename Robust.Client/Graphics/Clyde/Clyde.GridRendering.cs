...
namespace Robust.Client.Graphics.Clyde
{
    internal partial class Clyde
    {
        ...
        private void EnsureZLevelRenderMaps(MapId mapId, Box2 worldAABB, Box2Rotated worldBounds)
        {
            ...
            if (!mapUid.IsValid() || !zLevels.TryGetMapData(mapUid, out _, out var network))
                return;

            zLevels.CollectRenderableMaps(
                mapUid,
                mapId,
                Math.Max(0, network.VisibleLevelsBelow),
                Math.Max(0, network.VisibleLevelsAbove),
                worldAABB,
                worldBounds,
