...
namespace Robust.UnitTesting.Shared.Timing
{
    ...
    sealed class GameLoop_Test : OurRobustUnitTest
    {
        ...
        [Test]
        public void TimingAdjustmentChangeMidTickPreservesFirstPeriodWhenMultipleTicksCrossed()
        {
            BaseSetup();
            ...
            loop.Render += (_, _) =>
            {
                ...
                {
                    gameTiming.TickTimingAdjustment = 0.1f;
                    elapsedVal = TimeSpan.FromSeconds(0.275);
                    return;
                }
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(gameTiming.CurTick, Is.EqualTo(new GameTick(3)));
                Assert.That(gameTiming.LastTick, Is.EqualTo(TimeSpan.FromSeconds(0.19)).Within(TimeSpan.FromTicks(1)));
                Assert.That(gameTiming.TickRemainderRealtime, Is.EqualTo(TimeSpan.FromSeconds(0.085)).Within(TimeSpan.FromTicks(1)));
                Assert.That(gameTiming.TickPhase, Is.EqualTo(0.085f / 0.09f).Within(0.0001f));
            });
