...
namespace Robust.Server.GameObjects
{
    ...
    public sealed class ServerEntityManager : EntityManager, IServerEntityManagerInternal
    {
        ...
        private Dictionary<IPlayerSession, PlayerLastSeen> _lastSeen = new Dictionary<IPlayerSession, PlayerLastSeen>();
        ...
        private void PlayerStatusChanged(object? sender, SessionStatusEventArgs eventArgs)
        {
            if (!_lastSeen.TryGetValue(eventArgs.Session, out var lastSeen))
            {
                lastSeen = new PlayerLastSeen();
                _lastSeen[eventArgs.Session] = lastSeen;
            }
            else
            {
                lastSeen.Clear();
        ...
        public List<EntityState> GetEntityStates(GameTick fromTick, IPlayerSession player, float range)
        {
            ...
            if (!_lastSeen.TryGetValue(player, out var data))
                return new List<EntityState>();
            ...
            foreach (var entity in GetEntitiesInRange(playerEnt, range))
            {
                ...
                var lastSent = data.GetLastSeen(entity.Uid);

                if (entity.LastModifiedTick <= lastSent ||
                ...
                data.Update(entity.Uid, entity.LastModifiedTick);
    ...
    internal sealed class PlayerLastSeen
    {
        private readonly Dictionary<EntityUid, GameTick> _lastSeenTick = new Dictionary<EntityUid, GameTick>();

        public void Update(EntityUid uid, GameTick lastSeen)
        {
            _lastSeenTick[uid] = lastSeen;
        }

        public GameTick GetLastSeen(EntityUid uid)
        {
            if (_lastSeenTick.TryGetValue(uid, out var tick))
            {
                return tick;
            }

            return GameTick.First;
        }
