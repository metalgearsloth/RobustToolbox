...
internal sealed partial class PvsSystem : EntitySystem
{
    ...
    {
        if (map is not { } mapUid || !_zLevels.TryGetMapData(mapUid, out _, out var network))
            return (0, 0);

        return (
            Math.Min(_pvsZLevelsBelow, Math.Max(0, network.VisibleLevelsBelow)),
            Math.Min(_pvsZLevelsAbove, Math.Max(0, network.VisibleLevelsAbove)));
    }
