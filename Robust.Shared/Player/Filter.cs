    ...
    public sealed class Filter
    {
        ...
        public Filter AddInRange(MapCoordinates position, float range, ISharedPlayerManager? playerMan = null, IEntityManager? entMan = null)
        {
            ...
            var originMap = mapSystem.TryGetMap(position.MapId, out var mapUid)
                ? mapUid.Value
                : EntityUid.Invalid;
            zLevels.TryGetMapData(originMap, out _, out var network);
            var visibleBelow = network == null
                ? 0
                : Math.Min(Math.Max(0, network.VisibleLevelsBelow), Math.Max(0, cfgMan.GetCVar(CVars.NetPvsZLevelsBelow)));
            var visibleAbove = network == null
                ? 0
                : Math.Min(Math.Max(0, network.VisibleLevelsAbove), Math.Max(0, cfgMan.GetCVar(CVars.NetPvsZLevelsAbove)));
            ...
            int visibleAbove)
        {
            ...
            // Positive means the viewer is above the origin, so the origin consumes its lower-level allowance.
