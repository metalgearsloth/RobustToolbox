            ...
            switch (message)
            {
                ...
                case FullInputCmdMessage fullInput:
                {
                    var handled = _callback?.Invoke(new PointerInputCmdArgs(session, entManager.GetCoordinates(fullInput.Coordinates),
                        fullInput.ScreenCoordinates, entManager.GetEntity(fullInput.Uid, session), fullInput.State, message));
                    return handled.HasValue && handled.Value;
                }
                default:
        ...
        public override bool HandleCmdMessage(IEntityManager entManager, ICommonSession? session, IFullInputCmdMessage message)
        {
            switch (message)
            {
                ...
                case FullInputCmdMessage fullInput:
                    ...
                    {
                        case BoundKeyState.Up:
                            return _disabled?.Invoke(session, entManager.GetCoordinates(fullInput.Coordinates), entManager.GetEntity(fullInput.Uid, session)) == true;
                        case BoundKeyState.Down:
                            return _enabled?.Invoke(session, entManager.GetCoordinates(fullInput.Coordinates), entManager.GetEntity(fullInput.Uid, session)) == true;
                    }
                    break;
            }
